...
<!-- if statement to check if the button is clicked -->
<?php
	// creating submission query
		$query = "SELECT firstname,lastname,email,region FROM Lawyer WHERE lawyer_id = '$userid' ";
		$execute = mysqli_query($link,$query)or die(mysqli_error($link));
		$result = mysqli_fetch_array($execute);
			// perform nothing
?>
		<h3 class="text-center" style="font-family: roboto;color: darkred;"> Reputation For Service Appreciation.</h3>
		<h6>*** By writing and submitting this review. You agree and approve that "This information was kindly written by you as our customer.  ***</h6>
			<div class="form-group">
				<label for="firstName"> Lawyer Full Name </label>
				<input type="text" class="form-control text-capitalize" placeholder="<?php echo $result['firstname']?> <?php echo $result['lastname']?>">
			</div>
			
			<div class="form-group">
				<label for="email"> Lawyer E-mail</label>
				<input type="email" class="form-control" placeholder="<?php echo $result['email']?>">
			</div>
			<div class="form-group">
				<label for="mobile"> Lawyer Address </label>
				<input type="text" class="form-control text-capitalize" placeholder="<?php echo $result['region']?>">
			</div>
			<!-- Question Evaluation -->
			...
			<div class="form-group">
				...
				<textarea placeholder="Type your comment here...." class="form-control" name="review" rows="2"></textarea>
			</div>
